...
def train_and_evaluate_model(
    ...
    print(
        f"Test MSE: {mse:.4f}, Test MAE: {mae:.4f}, Test MAPE: {mape:.4f}"
    )
...
def main():
    ...
    positional_encoding_types = ["vanilla", "exponential_smoothing"]
    ...
    for residual_connection_type in residual_connection_types:
        for positional_encoding_type in positional_encoding_types:
            if positional_encoding_type == "exponential_smoothing":
                alphas = [0.1, 0.3, 0.5, 0.7, 0.9]
            else:
                alphas = [None]  # Placeholder for vanilla positional encoding
            ...
            for use_arima in use_arima_options:
                for use_es in use_es_options:
                    for garch_order in garch_orders:
                        for alpha in alphas:
                            print(
                                f"Training with Residual Connection Type: {residual_connection_type}, "
                                f"Positional Encoding Type: {positional_encoding_type}, Alpha: {alpha}, "
                                f"Use ARIMA: {use_arima}, Use ES: {use_es}, GARCH Order: {garch_order}"
                            )
                            ...
                            results.append(
                                {
                                    ...
                                    "Positional Encoding Type": positional_encoding_type,
                                    "Alpha": alpha,
                                    "Use ARIMA": use_arima,
                                    "Use ES": use_es,
