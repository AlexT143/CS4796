...
def train_arima(data):
    ...
    for i in range(data.shape[1]):
        model = auto_arima(data[:, i], seasonal=False, suppress_warnings=True)
        models.append(model)
    return models
...
def train_exponential_smoothing(data, trend='add', seasonal='add', freq=None):
    ...
    for i in range(data.shape[1]):
        ...
        series = pd.Series(data[:, i], index=pd.date_range(start='2010-01-01', periods=len(data), freq=freq))
...
def get_arima_forecast(models, steps, data):
    ...
    for i, model in enumerate(models):
        try:
            ...
            print(f"ARIMA forecast failed for column {i}: {str(e)}. Using last observed value as forecast.")
            last_value = data[-1, i]
