...
def train_and_evaluate_model(
    epochs, batch_size, ar_order, ma_order, volatility_type, num_runs
):
    results = []
    ...
    for run in range(num_runs):
        ...
        print(
            f"Test Loss for ARIMA-Inspired Transformer with AR order {ar_order}, MA order {ma_order}, Volatility {volatility_type}: {test_loss}"
        )
...
def main():
    ...
    ar_orders = [ 2]
    ma_orders = [ 1, 2, 3, 4]
    volatility_types = ["volatile", "involatile", "inline"]  # Add your volatility types here
    num_runs = (
        1  # Number of times to run the training and evaluation for each combination
    )
        ...
        for volatility_type in volatility_types:
            for ar_order in ar_orders:
                for ma_order in ma_orders:
                    print(
                        f"Training with AR order: {ar_order}, MA order: {ma_order}, Volatility: {volatility_type}"
                    )
