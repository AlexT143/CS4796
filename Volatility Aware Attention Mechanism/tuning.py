...
def train_and_evaluate_model(epochs, batch_size, garch_order, volatility_type, num_runs):
    results = []
    ...
    for run in range(num_runs):
        try:
            ...
            print(f"Test Loss for Volatility-Aware Transformer with GARCH order {garch_order}, Volatility {volatility_type}: {test_loss}")
            ...
            print(f"Caught InvalidArgumentError for GARCH order {garch_order}, Volatility {volatility_type}: {str(e)}")
            continue
...
def main():
    ...
    fieldnames = ["run", "garch_order", "volatility_type", "loss"]
    ...
    with open(csv_file, mode="a", newline="") as file:
        ...
        for p_order in p_orders:
            for q_order in q_orders:
                garch_order = (p_order, q_order)
                for volatility_type in volatility_types:
                    if file_exists:
                        data = pd.read_csv(csv_file)
                        model_count = len(data[(data["garch_order"] == str(garch_order)) & (data["volatility_type"] == volatility_type)])
                        num_additional_runs = 10 - model_count
                    else:
                        num_additional_runs = 10

                    if num_additional_runs > 0:
                        print(f"Training {num_additional_runs} additional models for GARCH order: {garch_order}, Volatility: {volatility_type}")
                        results = train_and_evaluate_model(epochs, batch_size, garch_order, volatility_type, num_additional_runs)

                        for _, result_row in results.iterrows():
                            writer.writerow(result_row.to_dict())
                    else:
                        print(f"Skipping training for GARCH order: {garch_order}, Volatility: {volatility_type} as 10 models are already trained.")
