...
def train_and_save_model(epochs, batch_size, garch_order, model_type):
    # Preprocess the data
    ...
    print(f"Test Loss for {model_type} Volatility-Aware Transformer: {test_loss}")

    # Save the model
    model_dir = f"models/{model_type}_volatility_transformer"
...
def main():
    ...
    involatile_params = (1, 1)  # GARCH(1, 1) for involatile market
    volatile_params = (3, 3)  # GARCH(3, 3) for volatile market
    inline_params = (2, 2)  # GARCH(2, 2) for in-line with market
    ...
    print("Feature Importance for Involatile Volatility-Aware Transformer:")
    for i, importance in enumerate(involatile_feature_importance):
        print(f"{feature_names[i]}: {importance}")

    print("\nFeature Importance for Volatile Volatility-Aware Transformer:")
    for i, importance in enumerate(volatile_feature_importance):
        print(f"{feature_names[i]}: {importance}")

    print("\nFeature Importance for Inline Volatility-Aware Transformer:")
