...
def create_lstm_model(input_shape, n_features):
    model = Sequential()
    model.add(LSTM(64, activation='relu', input_shape=(input_shape[0], n_features)))
    model.add(Dense(n_features))  # Update the output layer to have the same number of units as features
    model.compile(optimizer=Adam(learning_rate=0.001), loss='mse')
    return model

def train_and_save_lstm(model, X_train, y_train, X_val, y_val, epochs, batch_size):
...
if __name__ == "__main__":
    ...
    lstm_model = create_lstm_model(input_shape=(X_train.shape[1], n_features), n_features=n_features)

    # Train and save the LSTM model
    epochs = 25
    batch_size = 32
    lstm_model = train_and_save_lstm(lstm_model, X_train, y_train, X_val, y_val, epochs, batch_size)

    # Make predictions using the trained LSTM model
    lstm_predictions = make_predictions(lstm_model, X_test)

    # Calculate permutation feature importance
    feature_importance = calculate_permutation_feature_importance(lstm_model, X_test, y_test, scaler)
